		...
		if (camera.stereoTargetEye == StereoTargetEyeMask.Both || camera.stereoTargetEye == StereoTargetEyeMask.Left) 
		{
			...
			for (int i = 0; i < nodeStatesCache.Count; i++)
			{
				...
				if (nodeState.nodeType == XRNode.LeftEye)
				{
                    ...
                    {
						Vector3 eyePos = camera.transform.TransformPoint(leftEyePosition);
					}
                    else
                    {
                    ...
                    {
						Quaternion eyeRot = camera.transform.rotation * leftEyeRotation;
					}
					else
					{
			...
			Matrix4x4 projectionMatrix = camera.GetStereoProjectionMatrix();
